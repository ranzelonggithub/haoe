...
<body>
	<form class="form-horizontal" action="{{ url('/home/user/pass_edit_do_go') }}" method="get"> 
		<div id='box'>
			<div class="form-group">
		    	<label for="ori_pass" class="col-sm-2 control-label">输入原密码:</label>
		    	<div class="col-sm-10">
		      	<input type="password" class="form-control" id="ori_pass" name="old_passWord" placeholder="输入原密码...">
		    	</div>
		  	</div>
		  	<div id='old_pass'></div>

		  	<div class="form-group">
		    	<label for="pass" class="col-sm-2 control-label">输入新密码:</label>
		    	<div class="col-sm-10">
		      	<input type="password" class="form-control" id="pass" name="passWord" placeholder="输入新密码...">
		    	</div>
		  	</div>
		  		<div id='pa'></div>
	...
	<script>
		//输入员密码 ajax判断
		$('#ori_pass').blur(function() {
			var text = $('#ori_pass').val() ;
			var id = $('#uid').val() ;
			// alert(text) ;
			$.get("{{ url('/home/user/pass_edit_do') }}",{"id":id,"text":text},function(msg) {
				$('#old_pass').html(msg) ;
				$('#old_pass').addClass('active1') ;
			});
